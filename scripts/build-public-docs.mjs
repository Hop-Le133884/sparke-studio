...
function page(path, title, description, markup, type = 'TechArticle') {
  ...
  const metadata = content.publicMetadata({ origin, path, title: `${title} · Sparke`, description, type, indexable, breadcrumbs })
...
const summary = 'Sparke is an open-source, agent-first workspace for structured web interfaces, slides, reports, wireframes, 3D scenes, and timeline video. People and agents share one revision-checked document through the web UI, REST, CLI, MCP, and experimental WebMCP.';
...
const homeFallback = `<main class="public-home-fallback"><header><p>SPARKE</p><h1>A design workspace for people and agents.</h1><p>${escape(summary)}</p><nav><a href="/guide">Beginner guide</a> · <a href="/docs">Documentation</a> · <a href="/docs/api">API reference</a> · <a href="${repositoryUrl}">MIT source</a></nav></header><section><h2>From a clear brief to a useful artifact</h2><p>Choose a template, describe the audience and outcome, inspect the live preview, refine objects and themes, then export or publish an intentional snapshot.</p><h2>One structured document</h2><p>Use REST, the dsa CLI, network MCP with API keys or OAuth, or supported browser WebMCP. Atomic revisions protect concurrent human and agent edits.</p><h2>Your tools, your hosting</h2><p>Bring your own text, image, speech, music, or video provider keys. Run on Cloudflare or self-host using Docker. Template selection and manual editing work without provider credentials.</p></section></main>`;
