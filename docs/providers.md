...
Text completions (design generation, brief interviews, community metadata) wait up to `PROVIDER_TEXT_TIMEOUT_MS` milliseconds, default 600000 and clamped between 30 seconds and 30 minutes, because reasoning models writing a complete document can take several minutes. Other provider requests keep a 120-second limit. A timeout during connection or while the response is still streaming returns `provider_timeout` (HTTP 502) and leaves the project unchanged; [providers.ts](../server/providers.ts) owns the bound.

Before a generated document is validated, [generation-repair.ts](../src/shared/generation-repair.ts) applies a small bounded set of coercions for common provider habits: a missing node `name` is derived from the node's text, component or type; opacity written as 0–100 becomes a fraction; the CSS style names `background`, `backgroundColor`, `borderColor`, `borderWidth` and, on text and icon nodes, `color` move onto the renderer keys `fill`, `stroke` and `strokeWidth` when those are unset; CSS flexbox words in layouts (`flex-start`, `space-around`, `horizontal`, `wrap`, …) become the accepted `align`, `justify`, `direction`, `mode` and `wrap` values; layout `gap` and `padding` given as a CSS-style array or a `"16px"` string become one non-negative number (the largest side), an image, video, audio or model3d `src` that is not an HTTPS URL, an owned asset path or a raster data URL is removed so the node renders the studio's media placeholder until real media is placed, and a `children` array nested on a node is flattened into the page's node list directly after its parent with `parentId` set ; and any parent without a layout whose `parentId` children only fit its box when read as parent-relative coordinates receives an absolute layout, since the format otherwise reads them as page-space, and is grown (never shrunk) to contain those children. Nothing else is repaired or invented; remaining schema violations return `invalid_generation` with the failing field paths in the message, and the server logs the full issue list. REST, MCP and CLI writes are not repaired; they must already satisfy the shared schema.
